...
const SupporterIcon = ({ active }: { active: boolean }) => (
  ...
  >
    ...
    />
    <path
      d="M6 21V19C6 17.8954 6.89543 17 8 17H16C17.1046 17 18 17.8954 18 19V21"
      stroke={active ? "#2563EB" : "#64748B"}
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M19 14.5C19.5 15 20 15 20 15C20.5 15 21.5 14.5 21.5 13.5C21.5 12.5 20.5 11.5 20 12.5C19.5 11.5 18.5 12.5 18.5 13.5C18.5 14.5 19 15 19 14.5Z"
      fill="#EF4444"
      stroke="#EF4444"
      strokeWidth="1.5"
      strokeLinejoin="round"
...
const NGOIcon = ({ active }: { active: boolean }) => (
  ...
  >
    ...
    />
    <path
      d="M21 14C21 14 20 11.5 18.5 11.5C17 11.5 16 14 16 14C16 14 17 16.5 18.5 16.5C20 16.5 21 14 21 14Z"
      fill="#F59E0B"
      stroke="#F59E0B"
      strokeWidth="1.5"
      strokeLinejoin="round"
...
const CorporateIcon = ({ active }: { active: boolean }) => (
  ...
  >
    ...
    />
    <path
      d="M18 18H20C21.1046 18 22 17.1046 22 16V14C22 12.8954 21.1046 12 20 12H18"
      stroke="#F97316"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
...
const accountTypes = [
  ...
  },
  {
    ...
    label: "NGO",
    Icon: NGOIcon,
    description: "I represent a non-profit organization.",
  },
  {
    value: "corporate",
    label: "Corporate",
    Icon: CorporateIcon,
    description: "I represent a corporate or CSR team.",
  },
] as const;
  ...
  return (
    <form action={formAction} className="space-y-5">
      {state.message && (
        <div
          role="alert"
          className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-800"
        >
          {state.message}
        </div>
      )}
          ...
          {accountTypes.map(({ value, label, Icon, description }) => {
            ...
            return (
              <label key={value} className="cursor-pointer relative group">
                ...
                />
                <div
                  className={`h-[120px] rounded-[14px] border ${isActive ? "border-[#2563EB] bg-[#EFF6FF]" : "border-slate-200 bg-white group-hover:border-slate-300"} p-3.5 transition-all duration-200 flex flex-col items-center text-center justify-center`}
                >
                  {/* Radio indicator inside card at top left */}
                  <div className="absolute top-3 left-3">
                    <div
                      className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border ${isActive ? "border-[#2563EB]" : "border-slate-300"}`}
                    >
                      {isActive && (
                        <div className="h-[8px] w-[8px] rounded-full bg-[#2563EB]" />
                      )}
                    </div>
                  </div>

                  <div
                    className={`flex shrink-0 items-center justify-center w-10 h-10 rounded-xl mb-2.5 ${isActive ? "bg-white shadow-sm" : "bg-slate-50"}`}
                  >
                    <Icon active={isActive} />
                  </div>

                  <span
                    className={`text-[13px] font-bold leading-none mb-1.5 ${isActive ? "text-[#1e3a8a]" : "text-slate-900"}`}
                  >
                    {label}
                  </span>
                  <span className="text-[10px] leading-[1.2] text-slate-500 max-w-[95%]">
                    {description}
                  </span>
                </div>
              </label>
      ...
      <div className="flex gap-1.5 h-[4px] w-full mt-1.5">
        <div
          className={`flex-1 rounded-full ${strength.score >= 1 ? strength.color : "bg-slate-100"}`}
        />
        <div
          className={`flex-1 rounded-full ${strength.score >= 2 ? strength.color : "bg-slate-100"}`}
        />
        <div
          className={`flex-1 rounded-full ${strength.score >= 3 ? strength.color : "bg-slate-100"}`}
        />
        <div
          className={`flex-1 rounded-full ${strength.score >= 4 ? strength.color : "bg-slate-100"}`}
        />
        <div
          className={`flex-1 rounded-full ${strength.score >= 5 ? strength.color : "bg-slate-100"}`}
        />
      </div>
      ...
      <div className="pt-2">
        <SubmitButton rightIcon={<ArrowRight className="h-4 w-4 ml-1.5" />}>
          Create Account
        </SubmitButton>
      </div>
