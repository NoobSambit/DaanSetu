...
const sectionDefinitions = [
  { title: 'Basic Information', description: 'Organization details, mission, and description', icon: Building2 },
  { title: 'Location', description: 'Where is your organization based?', icon: MapPin },
  { title: 'Impact Focus', description: 'Your causes, programs, and impact areas', icon: HeartHandshake },
  { title: 'Trust & Verification', description: 'Legal details and verification documents', icon: ShieldCheck },
  { title: 'Social Presence', description: 'Website and social media links (optional)', icon: Globe2 },
  { title: 'Discoverability', description: 'Visibility preferences and opportunities', icon: Eye },
]
...
function AssetUploader({
  ...
  const [uploadState, setUploadState] = useState<AssetUploadState>(initialPath ? 'success' : 'idle')
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      if (previousPath && previousPath !== initialAssetPath && previousPath !== result.path) {
    ...
    <div>
      ...
      >
        {preview ? (
          ...
          <img src={preview} alt={`${label} preview`} className={`h-28 w-full object-cover ${uploadState === 'uploading' ? 'opacity-60' : ''}`} />
        ) : (
          <span className="flex flex-col items-center gap-2 text-center">
            ...
            <span className="text-xs font-medium text-slate-400">JPEG, PNG, or WebP up to 5 MB</span>
          </span>
        )}
      ...
      {status && (
        <p
          className={`mt-2 flex items-start gap-1.5 text-xs font-semibold ${
            uploadState === 'error' ? 'text-red-600' : uploadState === 'success' ? 'text-emerald-700' : 'text-slate-500'
          }`}
          role={uploadState === 'error' ? 'alert' : 'status'}
          aria-live="polite"
        >
          {uploadState === 'success' && <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />}
          {uploadState === 'error' && <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />}
          {uploadState === 'uploading' && <Loader2 className="mt-0.5 h-3.5 w-3.5 shrink-0 animate-spin" />}
          <span>{status}</span>
        </p>
      )}
    </div>
...
function VerificationDocuments({
  ...
  if (!verificationId) {
    return <p className="rounded-lg bg-amber-50 p-4 text-sm font-medium text-amber-900">Save the legal details once to enable private document uploads.</p>
  }
  ...
  return (
    <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5">
      ...
      <p className="text-xs font-medium text-slate-500">PDF, JPEG, or PNG. Maximum 10 MB. Documents are private.</p>
      
      {documents.length > 0 && (
        <div className="mt-4 space-y-2">
          {documents.map((document) => (
            <div key={document.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-3 text-sm">
              <span className="truncate pr-3 font-medium text-slate-700">{document.original_name}</span>
              {!disabled && <button type="button" onClick={() => remove(document.id)} className="font-semibold text-red-600 hover:text-red-700">Remove</button>}
            </div>
          ))}
        </div>
      )}
      {status && <p className="text-sm font-medium text-slate-600" aria-live="polite">{status}</p>}
    </div>
  )
}
...
export default function NgoProfileForm({
  ...
  const [liveLogoPreview, setLiveLogoPreview] = useState(publicAssetUrl(initialProfile.logoPath ?? null))
  ...
  const [locationDetectionState, setLocationDetectionState] = useState<LocationDetectionState>('idle')
  ...
  useEffect(() => {
    function cleanupUnsavedAssets() {
      if (formSubmittingRef.current || pendingAssetPathsRef.current.size === 0) return
  ...
  async function detectCurrentLocation() {
    if (!('geolocation' in navigator)) {
      ...
      setLocationMessage('Your browser does not support location detection. You can enter coordinates manually.')
      return
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const nextLatitude = Number(position.coords.latitude.toFixed(6)).toString()
        const nextLongitude = Number(position.coords.longitude.toFixed(6)).toString()
        const accuracy = Math.round(position.coords.accuracy)
        setLatitude(nextLatitude)
        setLongitude(nextLongitude)
        setLocationDetectionState('success')
        setLocationMessage(`Location detected with about ${accuracy}m accuracy. Review before saving.`)
      },
      (error) => {
        setLocationDetectionState('error')
        if (error.code === error.PERMISSION_DENIED) {
          setLocationMessage('The browser did not return location access. Check the site controls/lock icon and Brave privacy settings, then try again.')
        } else if (error.code === error.POSITION_UNAVAILABLE) {
          setLocationMessage('Your location is currently unavailable. Try again or enter coordinates manually.')
        } else if (error.code === error.TIMEOUT) {
          setLocationMessage('Location detection timed out. Try again or enter coordinates manually.')
        } else {
          setLocationMessage('Could not detect location. You can enter coordinates manually.')
        }
      },
      {
    ...
    <div className="mx-auto grid max-w-[1400px] gap-8 lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)_280px]">
      ...
      <aside className="space-y-8 lg:sticky lg:top-8 lg:self-start">
        ...
        <div>
          ...
          <p className="mt-4 text-sm leading-relaxed text-slate-600">
            Your profile becomes your public identity on DaanSetu. Donors, volunteers, CSR teams, and communities will discover and evaluate your organization through this page.
          </p>
        </div>

        <div className="relative space-y-6 before:absolute before:inset-y-2 before:left-[19px] before:w-[2px] before:bg-slate-100">
          {[
            { step: 'Step 1', title: 'Organization Details', desc: 'Foundation information and mission.', icon: Building2, color: 'text-blue-600', bg: 'bg-blue-50', ring: 'ring-blue-50' },
            { step: 'Step 2', title: 'Verification & Trust', desc: 'Legal registration and compliance.', icon: ShieldCheck, color: 'text-emerald-600', bg: 'bg-emerald-50', ring: 'ring-emerald-50' },
            { step: 'Step 3', title: 'Causes & Programs', desc: 'Areas of impact and initiatives.', icon: HeartHandshake, color: 'text-purple-600', bg: 'bg-purple-50', ring: 'ring-purple-50' },
            { step: 'Step 4', title: 'Visibility & Growth', desc: 'Become discoverable to supporters.', icon: BarChart3, color: 'text-orange-600', bg: 'bg-orange-50', ring: 'ring-orange-50' },
          ].map((s, i) => (
            <div key={i} className="relative flex gap-4">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${s.bg} ${s.color} ring-4 ring-white`}>
                <s.icon className="h-5 w-5" />
              </div>
              <div className="pt-0.5">
                <p className={`text-xs font-bold ${s.color}`}>{s.step}</p>
                <p className="mt-0.5 font-bold text-slate-900">{s.title}</p>
                <p className="mt-1 text-xs font-medium text-slate-500">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="p-5">
            <p className="mb-4 text-xs font-bold uppercase tracking-wider text-slate-900">Profile Preview</p>
            <div className="flex gap-4">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-slate-100 text-slate-400">
                {liveLogoPreview ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={liveLogoPreview} alt="" className="h-full w-full object-cover" />
                ) : (
                  <Building2 className="h-8 w-8" />
                )}
              </div>
              <div>
                <h4 className="font-bold text-slate-900">{initialProfile.displayName || 'Your organization'}</h4>
                ...
                <p className="mt-2 line-clamp-2 text-xs font-medium text-slate-500">
                  {initialProfile.tagline || 'Your mission and impact will appear here.'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center gap-1.5 text-xs font-medium text-slate-500">
              ...
              {initialProfile.city && initialProfile.state ? `${initialProfile.city}, ${initialProfile.state}, ${initialProfile.countryCode}` : 'Location pending'}
            </div>

            <div className="mt-5 space-y-4">
              <div>
                <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-900">Primary Cause</p>
                <span className="inline-block rounded bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">
                  {initialProfile.primaryCause ? NGO_CAUSE_LABELS[initialProfile.primaryCause as keyof typeof NGO_CAUSE_LABELS] : 'Pending'}
                </span>
              </div>

              <div>
                <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-900">Impact Areas</p>
                <div className="flex flex-wrap gap-2">
                  {initialProfile.impactAreas?.length ? (
                    initialProfile.impactAreas.slice(0, 3).map((area: string) => (
                      <span key={area} className="inline-block rounded-full border border-slate-200 px-2.5 py-1 text-[11px] font-bold text-slate-600">
                        {area.split('-').map(p => p[0].toUpperCase() + p.slice(1)).join(' ')}
                      </span>
                    ))
                  ) : (
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 border-t border-slate-100 pt-5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-900">Volunteer</p>
                  <p className="text-[11px] font-medium text-slate-500">Opportunities</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-900">Fundraising</p>
          ...
          <div className="border-t border-slate-100 bg-slate-50 p-3 text-center">
            {ngoId && profileStatus === 'published' ? (
              <a href={`/ngos/${ngoId}`} className="inline-flex min-h-8 items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-700">
                View Full Profile Preview <ArrowRight className="h-3.5 w-3.5" />
              </a>
            ) : (
              <span className="inline-flex min-h-8 items-center text-xs font-bold text-slate-400" aria-disabled="true">
      ...
      <section className="min-w-0">
        <div className="mb-6">
          ...
          <h2 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Tell us about your organization</h2>
          <p className="mt-2 font-medium text-slate-600">Help supporters understand your mission and impact.</p>
        </div>
        ...
        <form action={formAction} className="space-y-4">
          ...
          {sectionDefinitions.map((section, index) => {
            ...
            if (isCompleted) headerColorClass = 'bg-emerald-50 text-emerald-600'
            else if (isExpanded) headerColorClass = 'bg-blue-50 text-blue-600'

            return (
              <div key={sectionNumber} className={`overflow-hidden rounded-2xl border bg-white transition-all ${isExpanded ? 'border-blue-600 shadow-md ring-1 ring-blue-600' : 'border-slate-200 hover:border-slate-300'}`}>
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${headerColorClass}`}>
                      <Icon className="h-6 w-6" />
                    </div>
                    <div>
                      <h3 className="text-base font-bold text-slate-900">{sectionNumber}. {section.title}</h3>
                      <p className="mt-0.5 text-sm font-medium text-slate-500">{section.description}</p>
                    </div>
                  </div>
                  <div>
                ...
                {isExpanded && (
                  <div id={`ngo-profile-section-${sectionNumber}`} className="border-t border-slate-100 p-6 pt-6">
                    <div className="space-y-5">
                        ...
                        <>
                          <div className="grid gap-4 sm:grid-cols-2">
                            <AssetUploader
                              ...
                              onBusyChange={(busy) => setAssetUploadsInFlight((current) => Math.max(0, current + (busy ? 1 : -1)))}
                            />
                            <AssetUploader
                              name="coverImagePath"
                              label="Cover image"
                        ...
                        <>
                          ...
                          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div>
                                <p className="text-sm font-bold text-slate-900">Map coordinates</p>
                                <p className="mt-1 text-xs font-medium text-slate-500">Use your device location or enter coordinates manually.</p>
                                <p className="mt-1 text-[11px] font-medium text-slate-400">If no popup appears, check the site controls/lock icon or Brave privacy settings.</p>
                              </div>
                              <button
                                type="button"
                                onClick={detectCurrentLocation}
                                disabled={locationDetectionState === 'detecting'}
                              ...
                              >
                                ...
                                {locationDetectionState === 'detecting' ? 'Detecting...' : 'Use my current location'}
                              </button>
                            </div>
                            <div className="mt-4 grid gap-4 sm:grid-cols-2">
                              <div>
                                ...
                                <input
                                  ...
                                  inputMode="decimal"
                                />
                                <FieldError name="latitude" errors={state.fieldErrors} />
                              </div>
                              <div>
                            ...
                            {locationMessage && (
                              <p
                                ...
                                role={locationDetectionState === 'error' ? 'alert' : 'status'}
                                aria-live="polite"
                              >
                                {locationDetectionState === 'error' && <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />}
                                {locationDetectionState === 'success' && <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />}
                                {locationDetectionState === 'detecting' && <Loader2 className="mt-0.5 h-3.5 w-3.5 shrink-0 animate-spin" />}
                                <span>{locationMessage}</span>
                              </p>
                            )}
                          </div>
                          <p className="text-xs font-medium text-slate-500">Coordinates are optional. Profiles without them will not appear on the map.</p>
                        </>
                      )}
                      ...
                      {step === 4 && (
                        <>
                          <div className="rounded-lg bg-slate-50 p-4 text-sm font-medium text-slate-700">Current status: <strong className="text-slate-900">{verificationStatus.replace('-', ' ')}</strong>. Legal details and documents are never shown publicly.</div>
                          ...
                          <div className="grid gap-2 sm:grid-cols-3">{[['has12a', '12A registered', verification?.has_12a], ['has80g', '80G approved', verification?.has_80g], ['hasFcra', 'FCRA registered', verification?.has_fcra]].map(([name, label, value]) => <label key={String(name)} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg border border-slate-200 px-4 text-sm font-medium text-slate-700 hover:bg-slate-50"><input type="checkbox" name={String(name)} defaultChecked={Boolean(value)} className="h-4 w-4 rounded border-slate-300 text-blue-600" disabled={verificationStatus === 'pending' || verificationStatus === 'verified'} /> {String(label)}</label>)}</div>
                      ...
                      {step === 6 && (
                        <fieldset className="space-y-3"><legend className={labelClass}>Visibility preferences</legend>{[['isDiscoverable', 'Show this profile in the NGO directory and search', initialProfile.isDiscoverable], ['acceptsDonations', 'Show donation actions on the public profile', initialProfile.acceptsDonations], ['acceptsVolunteers', 'Show volunteer availability on the public profile', initialProfile.acceptsVolunteers]].map(([name, label, value]) => <label key={String(name)} className="flex min-h-14 cursor-pointer items-center gap-3 rounded-xl border border-slate-200 px-4 text-sm font-medium text-slate-700 hover:bg-slate-50"><input type="checkbox" name={String(name)} defaultChecked={value !== false} className="h-4 w-4 rounded border-slate-300 text-blue-600" /><span>{String(label)}</span></label>)}</fieldset>
                      )}
                    </div>
          ...
          <p className="mt-4 flex items-center justify-center gap-2 text-xs font-medium text-slate-500">
            <Lock className="h-3.5 w-3.5" /> Your information is secure and will never be shared without your permission.
          </p>
        </form>
      </section>
      ...
      <aside className="hidden space-y-6 lg:sticky lg:top-8 lg:block lg:self-start">
          ...
          <div className="mt-6 flex flex-col items-center">
            <div className="relative flex h-32 w-32 items-center justify-center rounded-full border-[10px] border-slate-100">
              ...
              <div className="text-center">
                ...
                <p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">Complete</p>
              </div>
            </div>
          </div>
            ...
            {sectionDefinitions.map((section, index) => {
              ...
              return (
                <div key={sectionNumber} className="flex items-center gap-3">
                  <div className={`h-3 w-3 shrink-0 rounded-full ${isCompleted ? 'bg-blue-600 ring-4 ring-blue-50' : isActive ? 'bg-blue-600 ring-4 ring-blue-50' : 'bg-slate-200'}`} />
                  <span className={`text-sm font-semibold ${isActive || isCompleted ? 'text-blue-600' : 'text-slate-500'}`}>
                    {section.title}
                  </span>
                </div>
              )
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50/50 p-5">
          ...
          <p className="mt-2 text-xs font-medium leading-relaxed text-slate-600">Our team is here to help you set up your organization profile.</p>
          <button type="button" className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white py-2.5 text-sm font-bold text-blue-600 shadow-sm hover:bg-slate-50">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-xs font-medium leading-relaxed text-slate-600">A complete profile builds trust, attracts more supporters, and creates greater impact.</p>
          <a href="#" className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-700">
