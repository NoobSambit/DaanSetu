...
export default function FollowNgoButton({ ngoId, initialFollowerCount = 0, isAuthenticated = false, className }: FollowNgoButtonProps) {
  ...
  const checkFollowStatus = async () => {
    try {
      const response = await fetch(`/api/follows/check?targetId=${ngoId}&targetType=ngo`)
      if (response.ok) {
  ...
  return (
    ...
    >
      <Heart className={`h-4 w-4 ${isFollowing ? 'fill-slate-500 text-slate-500' : ''}`} />
      <span>{isLoading ? 'Loading...' : isFollowing ? 'Following' : 'Follow Organization'}</span>
    </button>
  )
}
