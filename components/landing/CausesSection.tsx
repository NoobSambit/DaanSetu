...
import {
  Search, GraduationCap, Heart, Utensils, Shield, TreePine, PawPrint, Droplets,
  ChevronRight, Clock,
} from "lucide-react";
...
const topCauses = [
  {
    ...
    progress: 78, raised: "₹12.4L", goal: "₹16L", daysLeft: 14,
    tag: "Education", tagColor: "bg-blue-100 text-blue-700",
  },
  {
    title: "Community Health Service",
    org: "Swasth India Initiative",
    progress: 62, raised: "₹8.2L", goal: "₹13L", daysLeft: 21,
    tag: "Healthcare", tagColor: "bg-red-100 text-red-700",
  },
  {
    title: "Clean Water Initiative",
    org: "Jal Jeevan Foundation",
    progress: 45, raised: "₹5.6L", goal: "₹12.5L", daysLeft: 30,
    tag: "Environment", tagColor: "bg-green-100 text-green-700",
  },
];
    ...
    <section className="section bg-white">
      <div className="container-custom">
        <motion.div
          ...
          className="mb-6"
        >
          <h2 className="text-2xl md:text-3xl font-bold text-heading mb-1.5">Causes Across India</h2>
          <p className="text-body text-sm">Find meaningful causes in your community</p>
        </motion.div>
          ...
          <div className="grid lg:grid-cols-[260px_1fr_320px] divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
            ...
            <div className="p-6">
              ...
              <div className="space-y-1">
                {categories.map((cat) => (
                  <button key={cat.label} className="w-full flex items-center gap-3 px-3 py-2.5 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-heading rounded-xl transition-colors group">
            ...
            <div className="p-6 flex flex-col">
              <h3 className="text-[15px] font-bold text-heading mb-1">Top Causes Near You</h3>
              <p className="text-[11px] text-body mb-5">Popular causes in your area</p>
                ...
                {topCauses.map((cause, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2.5">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${cause.tagColor}`}>{cause.tag}</span>
                      <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-medium">
                        <Clock className="w-3 h-3" />{cause.daysLeft} days left
                      </div>
                    </div>
                    <h4 className="text-[13px] font-bold text-heading mb-1">{cause.title}</h4>
                    <p className="text-[11px] text-slate-500 mb-3">{cause.org}</p>
                    
                    <div className="w-full bg-slate-100 rounded-full h-2 mb-2">
                      <div className="bg-[#2563eb] h-2 rounded-full" style={{ width: `${cause.progress}%` }} />
                    </div>
                    <div className="flex justify-between text-[11px]">
                      <span className="text-[#1e293b] font-bold">{cause.raised} <span className="text-slate-500 font-normal">raised</span></span>
                      <span className="text-slate-600 font-medium">{cause.progress}%</span>
                    </div>
                  </motion.div>
                ))}
              </div>
