...
test('requires a strong password without silently trimming it', () => {
  assert.equal(validatePassword('Short1!'), 'Password must be at least 12 characters.')
  assert.equal(
...
test('accepts only self-service account types', () => {
  ...
  if (!invalid.success) {
    assert.equal(invalid.fieldErrors.accountType, 'Choose a valid account type.')
  }
})
...
test('allows internal return paths and blocks open redirects', () => {
  assert.equal(getSafeRedirectPath('/dashboard?tab=impact'), '/dashboard?tab=impact')
