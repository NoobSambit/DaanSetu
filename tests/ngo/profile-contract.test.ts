...
const completeProfile = {
  ...
  vision:
    "A just and equitable society where every child can reach their full potential.",
  theoryOfChange:
    "Community-led education support improves attendance, learning outcomes, and confidence.",
...
test("requires all core sections before publishing", () => {
  ...
  if (!incomplete.success) {
    assert.equal(
      incomplete.fieldErrors.mission,
      "Describe your organization mission.",
    );
    assert.equal(
      incomplete.fieldErrors.impactAreas,
...
test("rejects negative public impact metrics before publishing", () => {
  ...
  if (!invalid.success) {
    assert.equal(
      invalid.fieldErrors.beneficiariesReached,
      "Beneficiaries reached cannot be negative.",
    );
    assert.equal(
      invalid.fieldErrors.volunteersEngaged,
...
test("marks verification complete only after submission or approval", () => {
  assert.equal(
    calculateNgoProfileCompletion({}, { verificationStatus: "draft" })
      .sectionComplete.verification,
    false,
  );
  assert.equal(
    calculateNgoProfileCompletion({}, { verificationStatus: "rejected" })
      .sectionComplete.verification,
    false,
  );
  assert.equal(
    calculateNgoProfileCompletion({}, { verificationStatus: "pending" })
      .sectionComplete.verification,
    true,
  );
  assert.equal(
    calculateNgoProfileCompletion({}, { verificationStatus: "verified" })
      .sectionComplete.verification,
    true,
  );
});
