...
const migration = readFileSync(
  new URL(
    "../../supabase/migrations/015_ngo_profile_redesign.sql",
    import.meta.url,
  ),
  "utf8",
);
const publicProfileContentMigration = readFileSync(
  new URL(
    "../../supabase/migrations/016_public_ngo_profile_content.sql",
    import.meta.url,
  ),
  "utf8",
);
...
test("public RLS exposes published profiles but keeps drafts owner-scoped", () => {
  assert.match(
    migration,
    /profile_status = 'published'\s+OR user_id = auth\.uid\(\)/,
  );
...
test("profile assets use a public NGO bucket with owner-scoped writes", () => {
  ...
  assert.match(
    migration,
    /\(storage\.foldername\(name\)\)\[2\] IN \('logo', 'cover'\)/,
  );
});

test("public NGO profile content migration adds rich profile fields", () => {
  assert.match(
    publicProfileContentMigration,
  ...
  assert.match(
    publicProfileContentMigration,
    /ngos_beneficiaries_reached_nonnegative/,
  );
});
...
test("public NGO profile content migration prepares dashboard-managed content tables", () => {
  ...
  assert.match(
    publicProfileContentMigration,
    /Public can read active NGO programs/,
  );
  assert.match(
    publicProfileContentMigration,
    /NGO owners manage gallery images/,
  );
});
...
test("public NGO asset storage supports future dashboard media folders", () => {
  assert.match(
    publicProfileContentMigration,
    /\(storage\.foldername\(name\)\)\[2\] IN \('logo', 'cover', 'gallery', 'programs', 'updates'\)/,
  );
});
