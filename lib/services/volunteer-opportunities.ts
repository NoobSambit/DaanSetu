...
export async function createVolunteerOpportunity(
  ...
  if (ngoError || !ngo) {
    throw new Error(
      "Publish your NGO profile before creating volunteer opportunities",
    );
  }
...
export async function getVolunteerOpportunity(
  id: string,
  supabaseClient?: SupabaseClient,
): Promise<OpportunityWithNGO | null> {
  const supabase = supabaseClient || getBrowserClient();
...
export async function getUserApplications(
  supabaseClient?: SupabaseClient,
): Promise<Array<VolunteerApplication & { opportunity: OpportunityWithNGO }>> {
  const supabase = supabaseClient || getBrowserClient();
  ...
  return data as Array<
    VolunteerApplication & { opportunity: OpportunityWithNGO }
  >;
}
