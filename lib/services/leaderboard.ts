...
export async function getTopDonors(
  ...
  data.forEach((donation: any) => {
    ...
    const user = Array.isArray(donation.users)
      ? donation.users[0]
      : donation.users;
...
// Get top NGOs
export async function getTopNGOs(
  limit = 10,
  supabaseClient?: SupabaseClient,
): Promise<NGOLeaderboardEntry[]> {
  const supabase = supabaseClient || getBrowserClient();
...
export async function getTopCorporates(
  ...
  const { data: campaigns, error: campaignsError } = await supabase.from(
    "corporate_campaigns",
  ).select(`
      id,
      corporate_id,
...
export async function getUserDonorRank(
  userId: string,
  supabaseClient?: SupabaseClient,
): Promise<number | null> {
...
export async function getUserVolunteerRank(
  userId: string,
  supabaseClient?: SupabaseClient,
): Promise<number | null> {
