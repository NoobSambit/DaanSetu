...
const publicationSchema = profileInputSchema.superRefine((profile, context) => {
  ...
  if (
    ...
    profile.foundingYear < 1800 ||
    profile.foundingYear > currentYear
  ) {
    context.addIssue({
  ...
  if (
    profile.publicEmail &&
    !z.string().email().safeParse(profile.publicEmail).success
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["publicEmail"],
  ...
  if (
    profile.publicPhone &&
    profile.publicPhone.replace(/\D/g, "").length < 8
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["publicPhone"],
...
export function canPublishNgoProfile(
  input: unknown,
): NgoProfileValidationResult {
...
export function calculateNgoProfileCompletion(
  ...
  const sectionComplete = {
    ...
    // Draft and rejected submissions are not complete. Completion reflects an
    // actual submission for review or a verified organization.
    verification:
      options.verificationStatus === "pending" ||
      options.verificationStatus === "verified",
    // Social presence remains optional for publication, but its progress card
    // is complete only when the organization provides a public contact or link.
    social:
      Boolean(
        profile.websiteUrl || profile.publicEmail || profile.publicPhone,
  ...
  const completedSections =
    Object.values(sectionComplete).filter(Boolean).length;
