...
function errorsForSection(fieldErrors: Record<string, string>, section: NgoProfileSection) {
  const allowed = new Set(sectionFields[section])
  return Object.fromEntries(Object.entries(fieldErrors).filter(([field]) => allowed.has(field)))
}
...
async function saveVerificationSection(
  ...
  if (latestSubmission && ['pending', 'verified'].includes(latestSubmission.verification_status)) {
    return {
      status: 'success',
      message: latestSubmission.verification_status === 'pending'
  ...
  if (intent !== 'save' && (!legalName || !registrationNumber || !registrationType)) {
    return {
      status: 'error',
      message: 'Complete the required legal details before continuing.',
      fieldErrors: {
        ...(!legalName ? { verificationLegalName: 'Enter the registered legal name.' } : {}),
        ...(!registrationNumber ? { registrationNumber: 'Enter the registration number.' } : {}),
        ...(!registrationType ? { registrationType: 'Select the registration type.' } : {}),
      },
    }
  }
  ...
  const result = existing
    ? await supabase.from('ngo_verifications').update(payload).eq('id', existing.id).select('id').single()
    : await supabase.from('ngo_verifications').insert(payload).select('id').single()

  if (result.error || !result.data) {
    return { status: 'error', message: 'Verification details could not be saved.' }
  }
  ...
  if (intent === 'submit-verification') {
    ...
    if (!count) {
      return {
        status: 'error',
        message: 'Upload at least one registration document before submitting for review.',
    ...
    if (error) return { status: 'error', message: 'Verification could not be submitted.' }
    return {
      status: 'success',
  ...
  return { status: 'success', message: 'Verification draft saved.', verificationId: result.data.id }
}
...
export async function saveNgoProfileAction(
  ...
  const step = Math.min(6, Math.max(1, Number(readString(formData, 'step')) || 1))
  ...
  if (!user || (await getUserRole(supabase, user.id)) !== 'ngo') {
    return { status: 'error', message: 'You need an NGO account to manage this profile.' }
  }
  ...
  if (!ngo) {
    const { data, error } = await supabase
      ...
      .single()
    if (error || !data) {
      return { status: 'error', message: 'Your organization draft could not be created.' }
    }
    ngo = data
  }
  ...
  if (section === 'verification') {
    const verificationState = await saveVerificationSection(supabase, ngo.id, formData, intent)
    savedVerificationId = verificationState?.verificationId
    if (verificationState?.status === 'error' || intent === 'submit-verification') {
      revalidatePath('/ngo/profile')
      return verificationState ?? { status: 'error', message: 'Verification could not be saved.' }
    }
  } else {
    let normalized
    try {
  ...
  if (intent === 'publish') {
    ...
    if (!validation.success) {
      return {
        ...
        nextStep: PROFILE_SECTIONS.findIndex((candidate) =>
          sectionFields[candidate].some((field) => validation.fieldErrors[field])
        ) + 1,
    ...
    const { error } = await supabase
      ...
      .eq('id', ngo.id)
    if (error) return { status: 'error', message: 'The profile could not be published.' }
