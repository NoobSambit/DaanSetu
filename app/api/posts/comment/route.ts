...
export async function POST(request: NextRequest) {
  try {
    ...
    if (post && post.author_id !== user.id) {
      ...
      if (userData) {
        await notifyPostCommented(post.author_id, userData.name, postId, post.title)
      }
    }

    return NextResponse.json({
      ...commentWithUser,
      user: Array.isArray(commentWithUser?.user) ? commentWithUser.user[0] : commentWithUser?.user
    })
  } catch (error) {
