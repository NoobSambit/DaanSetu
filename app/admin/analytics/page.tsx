    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">Total AI Flags</p>
                <p className="text-3xl font-bold text-orange-600">{analytics.aiFlags.total}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">High Confidence Flags</p>
                <p className="text-3xl font-bold text-red-600">{analytics.aiFlags.highConfidence}</p>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Top 10 NGOs by Donations Received</h2>
          {analytics.topNGOs.length > 0 ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-gray-700">NGO Name</th>
                    <th className="text-right py-3 px-4 font-semibold text-gray-700">Total Received</th>
                  </tr>
                </thead>
                <tbody>
                  {analytics.topNGOs.map((ngo, index) => (
                    <tr key={ngo.id} className="border-b border-gray-100 hover:bg-gray-50">
            ...
            <p className="text-gray-500 text-center py-12">No NGO data available</p>
          )}
        </div>

        {/* Regional Heatmap (Simplified Table View) */}
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Donation Activity by Region</h2>
          {analytics.donationsByRegion.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {analytics.donationsByRegion.map((region) => {
                const maxAmount = Math.max(...analytics.donationsByRegion.map(r => r.amount))
                ...
                return (
                  <div key={region.region} className={`${bgColor} text-white p-4 rounded-lg`}>
                    <p className="font-semibold text-sm">{region.region}</p>
                    <p className="text-xl font-bold mt-1">₹{(region.amount / 1000).toFixed(0)}K</p>
                  </div>
                )
              })}
            </div>
