...
import { getPartnershipRequestsForCampaign, updatePartnershipRequestStatus } from '@/lib/services/partnerships'
...
export default function CorporateCampaignDetailPage() {
  ...
  const [campaign, setCampaign] = useState<CorporateCampaignWithProfile | null>(null)
  const [partnerships, setPartnerships] = useState<PartnershipRequestWithDetails[]>([])
    ...
    try {
      ...
      if (user) {
        ...
        if (profile && profile.id === campaignData.corporate_id) {
          setIsOwner(true)
          const partnershipData = await getPartnershipRequestsForCampaign(params.id as string)
  ...
  async function handlePartnershipAction(requestId: string, action: 'accepted' | 'rejected') {
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <Link href="/corporate/campaigns" className="text-blue-600 hover:text-blue-700 font-medium">
            ← Back to Campaigns
          </Link>
        </div>
          ...
          <div className="p-8">
            ...
            <div className="mb-8">
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-medium text-gray-900">{getProgress().toFixed(0)}%</span>
              </div>
            ...
            <div className="border-t border-gray-200 pt-6 mb-8">
              <h2 className="text-xl font-semibold text-gray-900 mb-3">Campaign Description</h2>
              <p className="text-gray-700 whitespace-pre-line">{campaign.description}</p>
            </div>

            {isOwner && partnerships.length > 0 && (
              <div className="border-t border-gray-200 pt-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Partnership Requests</h2>
                <div className="space-y-4">
                  {partnerships.map((partnership) => (
                    <div key={partnership.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="font-semibold text-gray-900">{partnership.ngo.name}</h3>
                          <p className="text-sm text-gray-600">
                            {partnership.ngo.category} • Created {new Date(partnership.created_at).toLocaleDateString()}
                          </p>
                        </div>
                        <span
                          className={`px-3 py-1 text-sm font-medium rounded-full ${
                      ...
                      {partnership.message && (
                        <p className="text-gray-700 text-sm mb-3">{partnership.message}</p>
                      )}
                      {partnership.status === 'pending' && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => handlePartnershipAction(partnership.id, 'accepted')}
                            className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700"
                          >
                            Accept
                          </button>
                          <button
                            onClick={() => handlePartnershipAction(partnership.id, 'rejected')}
