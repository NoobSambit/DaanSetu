...
import { getPlatformStats, getDonationsOverTime, getCampaignsOverTime, getVolunteerGrowth } from '@/lib/services/analytics'
...
export default function AnalyticsPage() {
  ...
  const [donationsData, setDonationsData] = useState<Array<{ date: string; amount: number }>>([])
  const [campaignsData, setCampaignsData] = useState<Array<{ date: string; count: number }>>([])
  const [volunteersData, setVolunteersData] = useState<Array<{ date: string; count: number }>>([])
  ...
  const loadAnalytics = async () => {
    try {
      setLoading(true)
      const [statsData, donationsTimeSeries, campaignsTimeSeries, volunteersTimeSeries] = await Promise.all([
        getPlatformStats(),
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Real-time insights into DaanSetu&apos;s impact</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">Total NGOs</p>
                <p className="text-3xl font-bold text-blue-600">{stats.totalNGOs}</p>
              </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Campaigns Created Over Time</h2>
            {campaignsData.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-12">No campaign data available yet</p>
            )}
          </div>

          {/* Volunteer Growth Over Time */}
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Volunteer Growth Over Time</h2>
            {volunteersData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={volunteersData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="count" stroke="#f97316" strokeWidth={2} name="Total Volunteers" />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <p className="text-gray-500 text-center py-12">No volunteer data available yet</p>
            )}
          </div>
        </div>
      </div>
